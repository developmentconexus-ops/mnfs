...
From this directory with Node `24.18.0`:
...
Deterministic tests run without live credentials. Tests requiring `TEST_DATABASE_URL` or `E2B_API_KEY` skip unless explicitly configured. Live A3/provider/E2B execution is opt-in, may create external effects/cost, and requires separately owned credentials and authority; never place credentials or generated auth files in the repository.

The package proves only its recorded Package-A properties. It is not Product implementation.
