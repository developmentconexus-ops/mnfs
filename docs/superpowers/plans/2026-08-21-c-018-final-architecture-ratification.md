...
**Goal:** Open C-018 as a bounded final architecture-ratification gate, prove that ratification cannot authorize Product implementation by itself, and preserve a separately operator-authorized closure path.

**Architecture:** Reuse the existing roadmap + Decision Register + repository guard model. C-018 introduces no ratification engine or new Product architecture; `docs/roadmap.md` owns mutable gate status, `docs/decisions/index.md` remains `NOT RATIFIED` while the gate is open, and `scripts/check-current-state.mjs` enforces phase continuity plus a deny-only Product implementation gate. Final ratification is a separate state transition and must not execute until explicit operator ratification is received.
...
- Do not run another whole-architecture Fable review unless current work introduces or exposes a material contradiction that requires independent challenge rather than a direct smallest-owner reopen.
...
- Modify: `scripts/check-current-state.mjs` — admit the ratified roadmap state only when Decision Register projection matches and Product remains blocked.
- Modify: `tests/repository/c018-ratification.test.mjs` — prove ratified-state continuity and decision projection.
...
- Produces: legal C-018 roadmap states `NOT RATIFIED | OPEN / RATIFICATION REVIEW` during the opening candidate; Product implementation is always `BLOCKED`; open C-018 requires all phases 3A–3O closed; open C-018 requires Decision Register C-018 to remain `NOT RATIFIED`.
...
- [ ] **Step 3: Run only these tests and verify RED for the intended reasons**
...
The first failure must be because current guard ignores C-018 overlap. The second must fail because the required deny-only message/behavior is absent; unrelated syntax/errors do not count as RED.

- [ ] **Step 4: Implement the minimal C-018 opening guard**
...
Remove the older weaker condition:
...
Keep the existing rule that a ratified C-018 cannot coexist with an unclosed phase; do not add a legal ratified state in this task.

- [ ] **Step 5: Re-run the focused repository contract test and verify GREEN**
...
- Produces: durable R1–R7 ratification contract, roadmap opening state, docs router entry, and proof that Decision Register remains `NOT RATIFIED`.

- [ ] **Step 1: Write the RED C-018 authority projection test**
...
test('documentation router reaches the C-018 durable contract', () => {
  ...
  assert.match(index, /phases\/c-018-final-architecture-ratification\.md/)
})
```
...
Expected: FAIL because the roadmap is still `NOT RATIFIED`, the durable C-018 phase file does not exist, and docs/index does not route it.

- [ ] **Step 3: Create the durable C-018 contract**

Create `docs/phases/c-018-final-architecture-ratification.md` with this bounded content:
...
- [ ] **Step 4: Open C-018 in the roadmap only**
...
Update only the final roadmap prose needed to say the exact current action is to evaluate C-018 continuity; explicitly state that Decision Register remains `NOT RATIFIED` and Product remains blocked.

- [ ] **Step 5: Route the durable C-018 contract from docs/index**

In `docs/index.md`, extend the durable phase route:
...
Do not add C-018 mutable status to docs/index.
...
- [ ] **Step 7: Confirm Decision Register is byte-unchanged for C-018**
...
**Files:**
- Delete from candidate tree: `docs/superpowers/specs/2026-08-21-c-018-final-architecture-ratification-design.md`
- Delete from candidate tree: `docs/superpowers/plans/2026-08-21-c-018-final-architecture-ratification.md`
- Verify all opening-candidate files from Tasks 1–2.

**Interfaces:**
- Consumes: completed opening candidate.
- Produces: merge-candidate tree containing only durable C-018 authority/mechanism/tests; planning artifacts remain only in branch history.

- [ ] **Step 1: Delete the two planning-only Markdown files from the candidate tree**
...
documentation reachability PASS
canonical current state PASS
3N architecture verification PASS
qualification provenance PASS
all repository tests PASS
exit code 0
```

- [ ] **Step 3: Inspect the exact diff against main**
...
`docs/decisions/index.md`, Product code, runtime, qualification harnesses and `docs/work/**` must not be present.

---

### Task 4: Publish the OPEN C-018 candidate for operator review

**Files:** none beyond Tasks 1–3.

**Interfaces:**
- Consumes: exact green opening candidate HEAD.
- Produces: one Draft PR against current `main`, with C-018 open but still NOT RATIFIED in Decision Register.
...
Expected base remains the current authoritative main. If main moved, stop and rebase/revalidate before publication.
...
```text
C-018 = OPEN / RATIFICATION REVIEW in roadmap only
Decision Register C-018 = NOT RATIFIED
Product implementation = BLOCKED
3A–3O = CLOSED
...
- [ ] **Step 4: Confirm CI on the exact PR HEAD**

Require GitHub Actions `verify = SUCCESS`. Read the job steps and confirm both `npm ci` and `npm run verify` succeeded. Do not infer green from an earlier commit.

- [ ] **Step 5: Review the candidate for a material contradiction**

Compare the exact PR diff to R1–R7. If no Product/owner/trust/qualification contradiction exists, do not create a Fable review by default. If a real contradiction exists, stop and return to the smallest owner; only use isolated Fable review when an adversarial independent challenge materially helps adjudication.

- [ ] **Step 6: STOP for explicit operator ratification**

Do not execute Task 5. Report the exact candidate HEAD, CI evidence, changed files, and the fact that C-018 remains NOT RATIFIED in the Decision Register.
...
- Produces: `C-018 = RATIFIED / OPERATOR RATIFIED` in roadmap, `C-018 = CURRENT / OPERATOR RATIFIED` in Decision Register, Product implementation still `BLOCKED`.
...
- [ ] **Step 2: Extend current-state guard only enough to admit explicit ratification**
...
Do not add any status that authorizes Product implementation.

- [ ] **Step 3: Update Decision Register only now**

Change the C-018 row to:
...
Do not change any C-001..C-017 disposition unless a material contradiction independently requires it.

- [ ] **Step 4: Update roadmap ratification state while keeping Product blocked**
...
The exact next action becomes Realization Planning under a separate authorized gate.

- [ ] **Step 5: Append durable result summary to C-018 contract**
...
- [ ] **Step 6: Re-run focused tests then full gate**
...
- [ ] **Step 7: Confirm exact closure diff contains no Product implementation**

```bash
git diff --name-only main...HEAD
```

Allowed closure changes remain documentation/guard/tests only. No Product source/runtime/qualification harness or `docs/work/**` file may appear.

- [ ] **Step 8: Update PR body with ratification evidence and STOP for merge authority**

Record exact closure HEAD + CI run. Do not merge until the operator separately authorizes merge.
