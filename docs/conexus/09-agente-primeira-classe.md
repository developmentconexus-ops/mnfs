...
registro C-005, executado por **loop leve no hub** (Vercel AI SDK como camada de dialeto
multi-provider, embrulhada na interface própria `ModelProviderAdapter`; domínio model-agnostic —
sem framework de agente e sem sandbox), enxergando o mundo
...
candidata isolada no T15 (C-002). **Distinção (emenda do ratificador):** rejeitar framework de
agente ≠ rejeitar lib de dialeto de provider — Vercel AI SDK adotada dia 1 (componente 4) porque
multi-provider é objetivo declarado do produto, e estrutura para objetivo declarado não é
overengineering.
...
| 15 | Modelo | Golden eval **Haiku 4.5 × Sonnet 5** (grounding, tool use, negações, adversariais); escolhe o mais barato que passa COM MARGEM; pinado por deployment; registra modelo solicitado + retornado (drift de alias invalida qualificação). Sem router dinâmico; sub-tarefas Haiku só com prova de eval. Cross-provider: **plumbing dia 1** (AI SDK, comp. 4); o que segue gatilhado é a QUALIFICAÇÃO de provider/modelo novo para produção — golden eval + política de classes de dado, sem número mágico. "Fase 1 US$0" = infra; inferência é BYOK pago. Política explícita de quais classes de dado vão a qual provider. |
...
| Qualificação de provider/modelo novo (Luna/Flash etc.) | Golden eval + política de classes de dado (plumbing multi-provider já existe: AI SDK) |
